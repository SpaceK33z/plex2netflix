...
function Plex2Netflix(options) {
    ...
    this.plexClient.query('/library/sections').then(function (result) {
        ...
        if (_this.options.librarySections) {
            return _this.findSpecificLibraries(result.MediaContainer.Directory);
        }

        return _this.findAllLibraries(result.MediaContainer.Directory);
    }).then(function (sections) {
        return executeSequentially(sections.map(function (section) {
            return function () {
                _this.reportOption('beforeSearchSection', section);
                return _this.getMediaForSection('/library/sections/' + section.key);
            };
        }));
    }).then(function () {
...
Plex2Netflix.prototype.getMediaMetadata = function (mediaUri) {
    ...
    return this.plexClient.query(mediaUri).then(function (result) {
        if (result.MediaContainer.Metadata && result.MediaContainer.Metadata.length) {
            var firstChild = result.MediaContainer.Metadata[0];
            ...
            return {
                ...
                title: _this2.filterTitle(result.MediaContainer.parentTitle || firstChild.originalTitle || firstChild.title),
...
Plex2Netflix.prototype.getMediaForSection = function (sectionUri) {
    ...
    return this.plexClient.query(sectionUri + '/all' + maybeAddYear).then(function (result) {
        var media = result.MediaContainer.Metadata;
